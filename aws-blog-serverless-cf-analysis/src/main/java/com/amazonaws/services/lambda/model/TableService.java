...
public class TableService {
    ...
    public void addPartitions(String tableName, Collection<Partition> partitions){
        addPartitions(tableName, partitions, false);
    }

    public void addPartitions(String tableName, Collection<Partition> partitions, boolean multiplePartition) {

        try (Connection conn = AthenaConnection.getConnection(); Statement statement = conn.createStatement()) {
            for (Partition partition : partitions) {
                String sql = "";

                if(multiplePartition) {
                    sql = String.format("ALTER TABLE %s ADD IF NOT EXISTS PARTITION (%s) LOCATION '%s'",
                            tableName, partition.spec(), partition.path());
                } else {
                    sql = String.format("ALTER TABLE %s ADD IF NOT EXISTS PARTITION (%s='%s') LOCATION '%s'",
